...
# 이변량 자료 : 두개의 변수에 대한 자료
# 다변량 자료 : 여러개의 변수에 대한 자료

# 두변수가 모두 질적 자료인 경우
# 첫번째 자료는 r개의 범주, 두번째 자료는 c개의 범주, 이러한 자료를 행렬의 요약한 표를 r x c분할표라 함
# 두변수가 모두 양적 자료인 경우
# (x1, y1), ... , (xn, yn)으로 표현

# 2.4.2 산점도(scatter plot)
# 이차원 평면에 각 변수의 값에 해당되는 점을 찍은 그림

# 2.4.3 표본 상관계수
# 두 변수의 선형적 함수관계를 나타내는 측도 (Sxx, Syy, Sxy)

# 2.4.4 허위상관과 잠복변수
# 강력범죄 발생건수에 따른 도시의 교회수를 봤을 때 높은 상관관계를 가짐
# '인구'라는 잠복변수로 인해 범죄건수와 교회수가 양의 상관관계를 가지는것처럼 잘못판단할 수 있음 이를 허위상관이라 함
